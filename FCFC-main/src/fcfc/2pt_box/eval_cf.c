  ...
  if (para->rank == para->root) {
    ...
    if (cf->rr) {
      if (cf->bintype == FCFC_BIN_SPI) {
        double fac = 2 * M_PI /
            ((double) conf->bsize[0] * conf->bsize[1] * conf->bsize[2]);
        for (int i = 0; i < cf->np; i++) {
          double dpi = cf->pbin_raw[i + 1] - cf->pbin_raw[i];
          for (int j = 0; j < cf->ns; j++) {
            cf->rr[j + i * cf->ns] = fac * dpi *
                (cf->sbin_raw[j + 1] * cf->sbin_raw[j + 1] -
                cf->sbin_raw[j] * cf->sbin_raw[j]);
          }
        }
      }
      else {      /* cf->bintype == FCFC_BIN_ISO or FCFC_BIN_SMU */
        double fac = 4 * M_PI / (3 * cf->nmu *
            (double) conf->bsize[0] * conf->bsize[1] * conf->bsize[2]);
        for (int i = 0; i < cf->ns; i++) {
          double rr = fac *
              (pow(cf->sbin_raw[i + 1], 3) - pow(cf->sbin_raw[i], 3));
